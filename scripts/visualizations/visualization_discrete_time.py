...
def create_controllers(delta_time, environment, dimension=2):
    ...
    lambda_obs = 2.2 * mass / delta_time
    lambda_DS = 1.0 * lambda_obs
    lambda_perp = 1.0 * lambda_obs
    controllers["$s_i = 2.2 m / \Delta t$"] = ObstacleAwarePassivController(
    ...
    lambda_obs = 2.05 * mass / delta_time
    lambda_DS = 1.0 * lambda_obs
    lambda_perp = 1.0 * lambda_obs
    controllers[r"$s_i = 2.05 m / \Delta t$"] = ObstacleAwarePassivController(
        lambda_dynamics=lambda_DS,
        lambda_remaining=lambda_perp,
        lambda_obstacle=lambda_obs,
        dimension=dimension,
    ...
    lambda_DS = 1.0 * lambda_obs
    lambda_perp = 1.0 * lambda_obs
    controllers[r"$s_i = 2.0 m / \Delta t$"] = ObstacleAwarePassivController(
        lambda_dynamics=lambda_DS,
        lambda_remaining=lambda_perp,
        lambda_obstacle=lambda_obs,
        dimension=dimension,
    ...
    lambda_obs = 0.5 * mass / delta_time
    lambda_DS = 1.0 * lambda_obs
    lambda_perp = 1.0 * lambda_obs
    controllers[r"$s_i = 0.5 m / \Delta t$"] = ObstacleAwarePassivController(
        lambda_dynamics=lambda_DS,
        lambda_remaining=lambda_perp,
        lambda_obstacle=lambda_obs,
        dimension=dimension,
...
def animation_discrete_system(save_animation=False):
    ...
    animator = AnimationCollisionAvoidance(
        ...
        dt_sleep=0.01,
        it_max=110,
        animation_name="discrete_control_linear_dynamics",
        file_type=".gif",
    )
    animator.setup(
        environment=environment,
