...
class MultiRobotAnimation(Animator):
    ...
    def update_step(self, ii: int, do_plotting: bool = True) -> None:
        """
        ...
        if self.new_disturbance:
            self.disturbance_pos_list = np.append(
                ...
                self.position_list[:, ii].reshape((self.DIM, 1)),
                axis=1,
            )
            self.new_disturbance = False
    ...
    def plot_anim_2D(self, ii):
        ...
        # atractor position
        atractor = (
            self.robot.controller.dynamic_avoider.initial_dynamics.attractor_position
        )
        self.ax.plot(
            atractor[0],
            atractor[1],
        ...
        if self.draw_qolo:
            ...
            if self.rotate_qolo:
                ...
                lenght_x_rotated = (
                    np.abs(np.cos(angle_rot)) * self.qolo_length_x
                    + np.abs(np.sin(angle_rot)) * self.qolo_length_y
                )
                lenght_y_rotated = (
                    np.abs(np.sin(angle_rot)) * self.qolo_length_x
                    + np.abs(np.cos(angle_rot)) * self.qolo_length_y
                )
            else:
    ...
    def plot_anim_3D(self, ii):
        ...
        for i, ax in enumerate(self.ax):
            ...
            for i, (disturbance, disturbance_pos) in enumerate(
                zip(
                    self.disturbance_list.transpose(),
                ...
                if i == 0:
                    breakpoint()
                    ax.arrow(
                        disturbance_pos[absciss],
                        disturbance_pos[ordinate],
                        disturbance[absciss] / draw_scaling,
            ...
            if self.draw_qolo:
                ...
                if self.rotate_qolo:
                    angle_rot = np.arctan2(
                        self.velocity_list[ordinate, ii],
    ...
    def on_press_not_use(self, event):
        ...
        if event.key.isspace():
            ...
            self.disturbance_list = np.append(
                self.disturbance_list, (np.array([[1.0], [0.0]])), axis=1
            )
            ...
            self.disturbance_list = np.append(
                self.disturbance_list, (np.array([[0.0], [1.0]])), axis=1
            )
            self.new_disturbance = True
    ...
    def run(self, save_animation: bool = False) -> None:
        ...
        self.fig.canvas.mpl_connect(
            "button_press_event", self.record_click_coord
        )  # modified
        self.fig.canvas.mpl_connect(
            "button_release_event", self.add_click_disturbance
        )  # added
    ...
    def artificial_disturbances_2D(self, ii):
        ...
        # if ii == 45:
        #     disturbance = np.array([-3.0, -2.0]) * self.disturbance_scaling
        #     self.robot.tau_e = disturbance
        #     self.disturbance_list = np.append(
        #         self.disturbance_list, disturbance.reshape(self.DIM, 1), axis=1
        #     )
